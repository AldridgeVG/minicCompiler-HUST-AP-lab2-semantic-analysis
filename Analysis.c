    ...
    if (T) {
        switch (T->nodeKind) {
            ...
            case FUNC_DEC:  //根据返回类型，函数名填写符号表,，，此时是函数定义
                rtn = fillSymbolTable(
                    T->type_id, createAlias(), LEV, T->type, 'F',
            ...
            case COMP_STM:
                ...
                symbolTable.index =
                    symbol_scope_TX
                        .TX[--symbol_scope_TX.top];  //删除该作用域中的符号
                break;
            case DEF_LIST:
                if (T->ptr[0]) {
            ...
            case VAR_DEF:  //处理一个局部变量定义,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
                ...
                while (T0) {  //处理所有VARDEC_LIST结点
                    ...
                    if (T0->ptr[0]->nodeKind == ID) {
                        rtn = fillSymbolTable(
                            T0->ptr[0]->type_id, createAlias(), LEV,
                            T0->ptr[0]->type, 'V',
                        ...
                        rtn = fillSymbolTable(
                            T0->ptr[0]->type_id, createAlias(), LEV,
                            T0->ptr[0]->type, 'A', T->scope);  //最后一个变量名
                        if (rtn == -1)
                            semantic_error(T0->ptr[0]->pos, T0->ptr[0]->type_id,
                                           "变量名重复定义");
                        ...
                        if (rtn == -1)
                            ...
                            strcpy(
                                opn1.id,
                                symbolTable.symbols[T0->ptr[0]->ptr[1]->place]
                                    .alias);
                            result.kind = ID;
                            strcpy(
                                result.id,
...
void Exp(struct node *T) {
    ...
    if (T) {
        switch (T->nodeKind) {
            ...
            case ASSIGNOP:
                if (T->ptr[0]->nodeKind != ID &&
                    T->ptr[0]->nodeKind != ARRAY_CALL &&
            ...
            case CONTINUE:
                if (T->break_num != 1) {
                    semantic_error(T->pos, T->type_id, "不允许在这个地方出现");
                }
                break;
            case BREAK:
                if (T->break_num != 1) {
